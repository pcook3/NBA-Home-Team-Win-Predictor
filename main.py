...
#dropping columns that won't be used in our predictive model
...
#renaming columns back to original name (changed automatically after merging)
...
#dropped the duplicate gameDate column (keeping the integer type column)
df = df.drop(columns=['gameDate'])


#converted the merged and cleansed data frame into a CSV file (commented out the line below)
